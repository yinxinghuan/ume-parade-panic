...
/**
 * Read the shell-owned login state at the moment a user-related action runs.
 * guest-shell may change this value after an external visitor signs in, so
 * user/profile gates must not rely on the module-load `isInAigram` snapshot.
 */
export function isInAigramNow(): boolean {
  return Boolean((window as any).Aigram?.isInAigram);
}

/** Current shell-owned player id, with the launch query as a compatibility fallback. */
